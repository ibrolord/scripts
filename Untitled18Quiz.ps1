...
function getEventLog()
{
   ...
   # $log1 = Get-EventLog -LogName Application  -newest 5| select TimeGenerated, message 
   $log1 = Get-EventLog -LogName $logName -newest $recentNo | select TimeGenerated, message 


    foreach($o in $log1){
        $richTextBox1.AppendText([string] "Text Generated: " + $o.timegenerated + "`r")
          $richTextBox1.AppendText("Message :" + $o.message + "`r") 
